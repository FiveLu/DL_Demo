...
def create_model():
    vgg_layers = [
        # unit1
        layers.Conv2D(64, input_shape=(32, 32, 3), kernel_size=[3, 3], padding='same', activation='relu',
        ...
        layers.Flatten(),
        layers.Dense(256, activation='relu', kernel_regularizer=regularizers.l2(regular_param)),
        layers.BatchNormalization(),
        layers.Dropout(0.5),
        layers.Dense(128, activation='relu', kernel_regularizer=regularizers.l2(regular_param)),
        layers.BatchNormalization(),
        layers.Dropout(0.5),
...
def cifar100_data(batchsz=64, shufflesz=1000,valid_p=0.2):
    ...
    db_valid=data.take(int(valid_p*50000)).map(preprocess).shuffle(shufflesz).batch(batchsz)
    db_train = data.skip(int(valid_p*50000)).map(preprocess).shuffle(shufflesz).batch(batchsz)
